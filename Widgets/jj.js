...
// 1. DPlayer video.url 字段（精确匹配，避免误取 pic 等其他字段）
//    匹配：url: 'https://...' 或 url: "https://..."
var dpUrlMatch = html.match(/\burl\s*:\s*['"]([^'"]*\.m3u8[^'"]*)['"]/);
if (dpUrlMatch && dpUrlMatch[1]) {
  videoUrl = dpUrlMatch[1];
}

// 2. HTML 注释里的 m3u8（javday 有 <!-- https://...m3u8 --> 备注）
if (!videoUrl) {
  var commentMatch = html.match(/<!--[^>]*?(https?:\/\/[^'">\s]+\.m3u8[^'">\s]*)/);
...
// 封面图：DPlayer pic 字段
var picMatch = html.match(/\bpic\s*:\s*['"]([^'"]+)['"]/);
if (picMatch && picMatch[1]) {
  posterUrl = normalizeUrl(picMatch[1]);
}
// 兜底：og:image
if (!posterUrl) {
  var ogMatch = html.match(/property=["']og:image["'][^>]*content=["']([^'"]+)["']/i)
    || html.match(/content=["']([^'"]+)["'][^>]*property=["']og:image["']/i);
  if (ogMatch && ogMatch[1]) posterUrl = ogMatch[1];
}

// 标题
var titleMatch = html.match(/<title[^>]*>([\s\S]*?)<\/title>/i);
var title = titleMatch ? titleMatch[1].replace(/\s*[-|｜–]\s*JAVDAY.*$/i, "").trim() : "";
...
return {
  ...
  videoUrl:   videoUrl,
  customHeaders: {
    // CDN (javday.homes) 只认站点根域名作为 Referer
    "Referer":    BASE_URL + "/",
    "User-Agent": UA,
  },
};
