...
// ---------- 从原始 HTML 字符串提取 m3u8 地址（正则，覆盖注释块）----------
// javday.app 将 m3u8 地址写在 HTML 注释内：
//   <!-- https://javday.homes/videos/.../index.m3u8 -->
//   <!--
//   <source src='https://javday.homes/videos/.../index.m3u8' type='application/x-mpegURL'/>
//   -->
function extractM3u8FromHtml(html) {
  // 优先：<source src='...'> 在注释块里
  var srcMatch = html.match(/\bsrc\s*=\s*['"]([^'"]*javday\.homes[^'"]*\.m3u8[^'"]*)['"]/i);
  if (srcMatch && srcMatch[1]) return srcMatch[1];

  // 次选：<!-- https://javday.homes/...index.m3u8 --> 纯 URL 注释
  var commentMatch = html.match(/<!--\s*(https?:\/\/[^\s>]*javday\.homes[^\s>]*\.m3u8)/i);
  if (commentMatch && commentMatch[1]) return commentMatch[1];

  // 兜底：任意位置含 javday.homes 的 m3u8 URL
  var anyMatch = html.match(/(https?:\/\/[^\s'"<>]*javday\.homes[^\s'"<>]*\.m3u8)/i);
  if (anyMatch && anyMatch[1]) return anyMatch[1];

  // DPlayer url 字段（兜底，适配其他播放器结构）
  var dplayerMatch = html.match(/url\s*:\s*['"]([^'"]+\.m3u8[^'"]*)['"]/i);
  if (dplayerMatch && dplayerMatch[1]) return dplayerMatch[1];
...
// ---------- loadDetail：直接从 detail 页原始 HTML 正则提取 m3u8 ----------
async function loadDetail(link) {
  try {
    link = normalizeUrl(link);
    var html = await fetchHtml(link);

    // 正则从原始 HTML 提取 m3u8（覆盖注释块，无需二次请求）
    var videoUrl = extractM3u8FromHtml(html);
