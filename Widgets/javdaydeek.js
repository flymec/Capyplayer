...
- 从 style=“background-image:url(…)” 中提取图片地址
  */
...
for (var i = 0; i < cards.length; i++) {
  ...
  // 卡片本身是 <a href="/videos/xxx/">
  // Widget.dom.attr 可直接对节点对象取属性，不需要重新 parse
...
- 加载视频详情，提取播放地址
- 
- 注意：JAVDay 的视频 CDN 域名是 javday.homes，与页面域名 javday.app 不同。
- CDN 校验 Referer 时需要传 CDN 域的 origin，否则返回 404。
- 因此 customHeaders.Referer 要用提取到的 videoUrl 的 origin，而不是详情页地址。
  */
  ...
  // 1. 从 DPlayer video: { url: ‘…’ } 中精确提取
  //    先截取 video: { … } 块，再从中取 url 值，避免跨行匹配失效
  // video: { … } 块内有换行，用 [\s\S] 跨行匹配
  ...
  // 2. 降级：页面中任意 .m3u8 地址（注释里也有备用地址，取第一个即可）
  if (!videoUrl) {
  ...
  // 提取 CDN 域名作为 Referer（如 https://javday.homes/）
  // CDN 校验的是请求来源域，用详情页 javday.app 域会被拒 (404)
